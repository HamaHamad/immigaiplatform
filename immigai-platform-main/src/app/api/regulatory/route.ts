...
export async function GET(req: NextRequest) {
  ...
  const userVisaTypes = new Set<string>(userCases.map((c: { visaCategory: string }) => c.visaCategory))
  ...
  const annotated = alerts.map((alert: any) => {
    ...
    const affectedCaseCount = alert.affectsAllCases
      ? userCases.length
      : userCases.filter((c: any) => affectedTypes.includes(c.visaCategory)).length
  ...
  const filtered = dashboard
    ? annotated.filter((a: any) => a.affectsUserCases || a.severity === 'critical')
    : annotated
